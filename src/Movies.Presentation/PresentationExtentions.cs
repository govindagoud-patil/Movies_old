...
namespace Movies.Presentation
{
    public static class PresentationExtensions
    {
        ...
        {
            route.MapGet("/api/movie",async (IMediator mediator,CancellationToken ct) =>
            {
                var movies = await mediator.Send(new GetMoviesQuery());
                return Results.Ok(movies);
            }).WithTags("Movies");

            route.MapGet("/api/movie/{id}", async (IMediator mediator,int id, CancellationToken ct) =>
            {
                var movie = await mediator.Send(new GetMovieByIdQuery(id),ct);
                return Results.Ok(movie);
            }).WithTags("Movies");

            route.MapPost("/api/movie", async (IMediator mediator, CreateMovieRequest createMovieRequest, CancellationToken ct) =>
            {
            ...
            route.MapPut("/api/movie{id}", async (IMediator mediator,int id, UpdateMovieRequest createMovieRequest, CancellationToken ct) =>
            {
